...
int main(){
	...
	for(int i = 1; i <= q; i++){
        ...
        long long ANS =  1ll * ans[query[i].r * (m - 1)] - ans[(query[i].l - 1) * (m - 1)] + 1ll * (n * (m - 1) - find2((query[i].l - 1) * (m - 1) + 1, query[i].r * (m - 1))) * query[i].h + find((query[i].l - 1) * (m - 1) + 1 , query[i].r * (m - 1));
